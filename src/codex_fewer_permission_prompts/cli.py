...
def propose_candidates(
    ...
    max_candidates: int,
    shell_wrapper: str,
    exclude_patterns: Iterable[Sequence[str]] = (),
) -> list[RuleCandidate]:
    counts, kinds = command_counts(records)
    candidates: list[RuleCandidate] = []
    seen_patterns: set[tuple[str, ...]] = set()
    existing_patterns = {tuple(pattern) for pattern in exclude_patterns}
    for command, count in counts.most_common():
        ...
        if pattern_is_covered(pattern, existing_patterns):
            continue
...
def pattern_is_covered(pattern: Sequence[str], existing_patterns: Iterable[Sequence[str]]) -> bool:
    candidate = tuple(pattern)
    for existing in existing_patterns:
        prefix = tuple(existing)
...
def render_block(candidates: Sequence[RuleCandidate]) -> str:
    ...
    lines.extend(render_rule_entries(candidates))
    lines.append(END)
    return "\n".join(lines) + "\n"


def render_rule_entries(candidates: Sequence[RuleCandidate]) -> list[str]:
    lines: list[str] = []
...
def append_to_sentinel(text: str, candidates: Sequence[RuleCandidate]) -> str:
    entries = "\n".join(render_rule_entries(candidates))
    if not entries:
        return text
    block_text, found = sentinel_text(text)
    if not found:
        return replace_sentinel(text, render_block(candidates))
    updated_block = block_text.replace(END, entries + "\n" + END, 1)
    return text.replace(block_text, updated_block, 1)
...
def load_or_generate_candidates(args: argparse.Namespace) -> list[RuleCandidate]:
    ...
    return propose_candidates(records, rules, args.min_count, args.max_candidates, args.shell_wrapper, exclude_patterns)
...
def cmd_propose(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not candidates:
        print("No new safe candidates met the current threshold. Existing matching rules were omitted.")
        return 0
...
def extract_prefix_patterns_from_rules(path: Path) -> set[tuple[str, ...]]:
    if not path.exists():
        return set()
    text = path.read_text(encoding="utf-8", errors="replace")
    return extract_prefix_patterns_from_text(text)


def extract_prefix_patterns_from_text(text: str) -> set[tuple[str, ...]]:
    patterns: set[tuple[str, ...]] = set()
    for match in re.finditer(r"prefix_rule\((.*?)\)", text, re.S):
        pattern = extract_list_field(match.group(1), "pattern")
        if pattern:
...
def cmd_apply(args: argparse.Namespace) -> int:
    ...
    existing_patterns = extract_prefix_patterns_from_text(old)
    candidates = load_or_generate_candidates(args)
    candidates = [candidate for candidate in candidates if not pattern_is_covered(candidate.pattern, existing_patterns)]
    if not candidates:
        print("No new safe candidates to apply. Existing matching rules were omitted.")
        return 0
    new = append_to_sentinel(old, candidates)
