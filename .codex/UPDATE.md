...
```text
请完全重启 Codex App，或新开 CLI 实例，让它重新扫描 ~/.agents/skills；重启后直接说“/fewer-permission-prompts”即可开始默认 dry-run 流程。不要用更新后尚未重启的旧 App 进程判断 / 菜单是否已刷新，因为旧进程可能缓存更新前的 skill 列表。
```
